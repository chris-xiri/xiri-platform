...
async function generatePersonalizedEmail(templateId, variables) {
  try {
    ...
    const prompt2 = `You are a professional email writer for Xiri Facility Solutions.
...
var analyzeVendorLeads = async (rawVendors, jobQuery, hasActiveContract = false) => {
  ...
  try {
    ...
    const prompt2 = template?.content.replace(/\{\{query\}\}/g, jobQuery).replace(/\{\{modeDescription\}\}/g, modeDescription).replace(/\{\{threshold\}\}/g, threshold.toString()).replace(/\{\{vendorList\}\}/g, vendorList);
...
var generateOutreachContent = async (vendor, preferredChannel) => {
  ...
  try {
    ...
    const prompt2 = template?.content.replace(/\{\{vendorName\}\}/g, vendor.companyName).replace(/\{\{specialty\}\}/g, vendor.specialty || "Services").replace(/\{\{campaignContext\}\}/g, campaignContext);
    const result = await model2.generateContent(prompt2);
...
var analyzeIncomingMessage = async (vendor, messageContent, previousContext) => {
  try {
    ...
    const template = templateDoc.data();
    const prompt2 = template?.content.replace(/\{\{vendorName\}\}/g, vendor.companyName).replace(/\{\{messageContent\}\}/g, messageContent).replace(/\{\{previousContext\}\}/g, previousContext).replace(/\{\{vendorId\}\}/g, vendor.id);
    const result = await model2.generateContent(prompt2);
    let text = result.response.text();
    text = text.replace(/^```json/gm, "").replace(/^```/gm, "").trim();
    const jsonContent = JSON.parse(text);
...
async function verifyDocument(docType, vendorName, specialty) {
  ...
  try {
    ...
    const prompt2 = template?.content.replace(/\{\{documentType\}\}/g, docType).replace(/\{\{vendorName\}\}/g, vendorName).replace(/\{\{specialty\}\}/g, specialty).replace(/\{\{requirements\}\}/g, requirements).replace(/\{\{ocrText\}\}/g, simulatedOcrText);
    const result = await model3.generateContent({
      contents: [{ role: "user", parts: [{ text: prompt2 }] }]
    });
