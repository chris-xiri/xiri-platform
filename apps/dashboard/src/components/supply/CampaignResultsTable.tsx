...
import { CheckCircle2, XCircle, Eye, ChevronDown, ChevronUp, ExternalLink, Phone, Globe, MapPin, X } from 'lucide-react';
...
export default function CampaignResultsTable({
    ...
    const handleBulkApprove = () => {
        selectedVendors.forEach(id => onApprove(activeCampaign.id, id));
        setSelectedVendors(new Set());
        setShowBulkApproveDialog(false);
    };

    const handleBulkDismiss = () => {
        selectedVendors.forEach(id => onDismiss(activeCampaign.id, id));
        setSelectedVendors(new Set());
        setShowBulkDismissDialog(false);
    };

    const handleCloseTab = (campaignId: string) => {
        setShowCloseTabDialog(campaignId);
    };

    const confirmCloseTab = () => {
        if (showCloseTabDialog) {
            onCloseCampaign(showCloseTabDialog);
            setShowCloseTabDialog(null);
            setSelectedVendors(new Set());
        }
    };
    ...
    return (
        <Card className="border-2 border-dashed border-blue-400/50 bg-blue-50/30 dark:bg-blue-950/20 shadow-sm mb-4">
            ...
            <div className="flex items-center border-b border-blue-200/50 dark:border-blue-800/50 bg-blue-50/50 dark:bg-blue-950/30 overflow-x-auto">
                <div className="flex items-center gap-0.5 px-2 py-1.5 flex-1 min-w-0">
                    ...
                    {campaigns.map((campaign) => (
                        ...
                        >
                            <span className="truncate">{campaign.query}</span>
                            ...
                            <button
                                ...
                                title={`Close "${campaign.query}" — dismisses remaining vendors`}
                            >
                                <X className="w-3 h-3" />
                            </button>
                        </div>
                ...
                <div className="flex items-center gap-1.5 px-2 flex-shrink-0">
                    <Badge variant="outline" className="text-[10px] border-amber-400 text-amber-600 dark:text-amber-400 whitespace-nowrap">
                        Preview only
                    </Badge>
            ...
            {expanded && activeCampaign && (
                <div className="px-4 py-1.5 text-xs text-muted-foreground border-b border-blue-100/50 dark:border-blue-900/30 bg-blue-50/30 dark:bg-blue-950/10">
                    <span className="font-medium text-foreground">"{activeCampaign.query}"</span>
                    {' '}in{' '}
                    <span className="font-medium text-foreground">{activeCampaign.location}</span>
                    {' '}— {activeCampaign.sourced} sourced, {activeCampaign.qualified} qualified, {activeCampaign.vendors.length} remaining
                </div>
            )}
            ...
            {expanded && (
                <CardContent className="p-0">
                                ...
                                {vendors.map((vendor, index) => (
                                    ...
                                    >
                                        <TableCell className="text-center py-2">
                                            <Checkbox checked={selectedVendors.has(vendor.id!)} onCheckedChange={(checked: boolean) => handleSelectVendor(vendor.id!, checked)} />
                                        </TableCell>
                                        <TableCell className="text-center text-xs text-muted-foreground py-2">{index + 1}</TableCell>
                                        <TableCell className="py-2">
                                            <div className="flex flex-col">
                                                <span className="font-medium text-sm truncate max-w-[200px]">{vendor.businessName}</span>
                                                {vendor.website && (
                                        ...
                                        <TableCell className="text-center py-2">
                                            <span className={`text-sm font-bold ${getScoreColor(vendor.fitScore)}`}>{vendor.fitScore || '—'}</span>
                                        </TableCell>
                                        <TableCell className="py-2">
                                            <div className="flex flex-wrap gap-1">
                                                {(vendor.capabilities || []).slice(0, 2).map((cap, i) => (
                                                    <Badge key={i} variant="secondary" className="text-[10px] px-1.5 py-0">{cap}</Badge>
                                                ))}
                                                {(vendor.capabilities || []).length > 2 && (
                                                    <Badge variant="outline" className="text-[10px] px-1 py-0">+{(vendor.capabilities || []).length - 2}</Badge>
                                                )}
                                            </div>
                                        </TableCell>
                                        <TableCell className="text-center py-2">
                                        ...
                                        <TableCell className="text-center py-2">
                                            <div className="flex items-center justify-center gap-1">
                                                <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-green-600 hover:text-green-700 hover:bg-green-50" onClick={() => onApprove(activeCampaign.id, vendor.id!)} title="Approve into CRM">
                                                    <CheckCircle2 className="w-4 h-4" />
                                                </Button>
                                                <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-red-500 hover:text-red-600 hover:bg-red-50" onClick={() => onDismiss(activeCampaign.id, vendor.id!)} title="Dismiss">
                    ...
                    <div className="md:hidden p-3 space-y-2">
                        {vendors.map((vendor, index) => (
                            <div key={vendor.id || index} className="border border-blue-200/50 dark:border-blue-800/50 rounded-lg p-3 bg-white dark:bg-card">
                                ...
                                <div className="flex items-center gap-2">
                                    <Button size="sm" className="flex-1 h-8 text-xs bg-green-600 hover:bg-green-700 text-white" onClick={() => onApprove(activeCampaign.id, vendor.id!)}>
                                        <CheckCircle2 className="w-3 h-3 mr-1" /> Approve
                                    </Button>
                                    <Button size="sm" variant="ghost" className="flex-1 h-8 text-xs text-red-600" onClick={() => onDismiss(activeCampaign.id, vendor.id!)}>
            ...
            <AlertDialog open={showBulkApproveDialog} onOpenChange={setShowBulkApproveDialog}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>This will save them to the CRM pipeline with "pending_review" status.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <AlertDialog open={showBulkDismissDialog} onOpenChange={setShowBulkDismissDialog}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>These vendors will be blacklisted and won't appear in future campaigns.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={handleBulkDismiss} className="bg-red-600 hover:bg-red-700 text-white">Dismiss</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
