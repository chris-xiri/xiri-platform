...
export default function AuditWizardPage() {
    ...
    const StepSchedule = () => {
        ...
        const toggleSlot = (dateStr: string) => {
            if (auditSlots.includes(dateStr)) {
                setAuditSlots(auditSlots.filter(s => s !== dateStr));
            } else if (auditSlots.length < 3) {
                setAuditSlots([...auditSlots, dateStr]);
            }
        };
        ...
        return (
            <div className="space-y-6 animate-fadeIn">
                <div>
                    <h2 className="text-2xl font-bold text-gray-900">When can we visit?</h2>
                    <p className="text-gray-600 mt-2">Select up to 3 preferred times for your site walkthrough</p>
                </div>

                <div className="space-y-4">
                    {slots.map((daySlot, dayIndex) => {
                        const dayName = format(daySlot.date, 'EEEE');
                        const dateStr = format(daySlot.date, 'MMM d');
                        const isToday = format(daySlot.date, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd');
                        const isTomorrow = format(daySlot.date, 'yyyy-MM-dd') === format(addDays(new Date(), 1), 'yyyy-MM-dd');
                        ...
                        return (
                            <div key={dayIndex} className="border border-gray-200 rounded-lg p-4">
                                <div className="flex items-baseline gap-2 mb-3">
                                    ...
                                    {isTomorrow && <span className="text-xs bg-sky-100 text-sky-700 px-2 py-0.5 rounded">Tomorrow</span>}
                                </div>
                                    ...
                                    {daySlot.times.map((timeSlot, timeIndex) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                disabled={auditSlots.length >= 3 && !selected}
                                                className={`py-2.5 px-3 rounded-lg border text-sm font-medium transition-all ${selected
                                                    ? 'bg-sky-600 text-white border-sky-600'
                                                    : 'bg-white border-gray-200 hover:border-sky-300 hover:bg-sky-50 disabled:opacity-40 disabled:cursor-not-allowed'
                                                    }`}
                                            >
                                                {timeSlot.label}
                                            </button>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                    <p className="text-sm text-gray-600">
                        {auditSlots.length === 0 ? 'No times selected' : `${auditSlots.length} of 3 times selected`}
                    </p>
                    <button
