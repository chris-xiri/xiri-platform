...
export default async function LocationsServiceHub({ params }: Props) {
    ...
    // Targeted high-value industries requested for local dominance
    ...
    const industries = seoData.industries.filter((i: any) => TARGET_INDUSTRIES.includes(i.slug));

    return (
        <div className="min-h-screen bg-white">
            <Hero
                title={`Commercial & Medical Cleaning in ${townName}, ${state.toUpperCase()}`}
                subtitle={location.localInsight || `Comprehensive facility management, dental office sanitation, and commercial cleaning trusted by businesses across ${location.region}.`}
            ...
            <div className="max-w-7xl mx-auto px-4 py-16 sm:px-6 lg:px-8 space-y-16">
                ...
                <div>
                    <div className="mb-8 text-center max-w-3xl mx-auto">
                        <h2 className="text-3xl font-bold text-gray-900 mb-4">Primary Services in {townName}</h2>
                ...
                {/* Section 2: Specialized Industry Cleaning (Dental, Urgent Care, Gyms, etc.) */}
                <div>
                    <div className="mb-8 text-center max-w-3xl mx-auto">
                        <h2 className="text-3xl font-bold text-gray-900 mb-4">Specialized Facility & Dental Office Cleaning</h2>
                        <p className="text-gray-600">
                            Tailored sanitation scopes engineered for OSHA, HIPAA, and industry-specific compliance standards in {townName}.
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {industries.map((ind: any) => (
                            <Link 
                                key={ind.slug} 
                                href={`/locations/${state}/${county}/${town}/${ind.slug}`}
                                className="flex flex-col p-6 bg-sky-50/50 border border-sky-100 rounded-xl hover:shadow-lg hover:border-sky-300 transition-all group"
                            >
                                <h3 className="text-xl font-bold text-gray-900 group-hover:text-sky-700 transition-colors mb-2">
                                    {ind.name || ind.heroTitle}
                                </h3>
                                <p className="text-sm text-gray-600 mb-4 flex-grow">
                                    {ind.heroSubtitle}
                                </p>
                                <span className="text-sky-600 font-medium text-sm flex items-center">
                                    Explore {ind.name || 'Industry'} Solutions <span className="ml-1">→</span>
                                </span>
                            </Link>
                        ))}
                    </div>
            ...
            {location.landmarks && location.landmarks.length > 0 && (
                <div className="bg-gray-50 border-t border-gray-200 py-12">
                    <div className="max-w-4xl mx-auto px-4 text-center">
                        <h3 className="font-semibold text-gray-900 mb-2">Proudly Serving Facilities Near</h3>
